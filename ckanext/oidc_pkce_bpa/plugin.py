...
def _user_has_auth0_identity(user: Optional[model.User]) -> bool:
    if not user:
        return False
    plugin_extras = getattr(user, "plugin_extras", None) or {}
    oidc_data = plugin_extras.get("oidc_pkce") or {}
...
class OidcPkceBpaPlugin(SingletonPlugin):
    ...
    def logout(self):
        session.pop(SESSION_ADMIN_LOGIN_TOKEN, None)
        session.pop(SESSION_ADMIN_LOGIN_TARGET, None)
        if session.pop(SESSION_AUTH0_LOGOUT, False):
            log.debug("Detected in-progress Auth0 logout; allowing CKAN logout to continue.")
            return None
        ...
        try:
            ck_logout_response = user_view.logout()
        finally:
